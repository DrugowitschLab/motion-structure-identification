...
def loss_fun(𝜃, df, mask, disp):
    ...
    u = (β * (df.iloc[:, :8] + b)).to_numpy() - 1 / 4 / mult
    normalizer = logsumexp(u, axis=1, keepdims=True)
    # ℓ = u.apply(lambda row: logsumexp(row[masks[df['target'][row.name]]]), axis=1) - normalizer
    # loss = -ℓ.sum()
    p = np.exp(u - normalizer)
    p = (α * mult + (1 - α) * p)
    loss = (p * mask).sum(axis=1)
...
if __name__ == '__main__':
    plot_prediction(file='exp1/pilot1_20190806141059_σ_R=6_σ_x=0_x1.csv', target='choice', method='SLSQP', x0=np.array([0.1, 0, 0, 0]))
