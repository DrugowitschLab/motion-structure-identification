...
def consistency(ax: plt.Axes):
    from pub.eLife.plot_fig2 import consistency
...
# def confidence(ax: plt.Axes, q: int = 10, Exp: Type[Data] = DataExp1):
#     ax.set_frame_on(False)
#     ax.set_xticks([])
#     ax.set_xlabel('')
#     ax.set_yticks([])
#     ax.set_ylabel('')
#     fig = plt.gcf()
#     gs = gridspec.GridSpecFromSubplotSpec(3, 2, subplot_spec=ax, wspace=0.2, hspace=0.3)
#     x, y = [[[], [], []], [[], [], []]], []
#     for pid in Exp.pids:
#         data = Exp(pid)
#         model = data.build_model(models.HumanObserver4Param)
#         y += list((model.df['confidence'] == 'high').astype(float))
#         L = -np.sort(-model.ideal_observer_log_posterior_odds().to_numpy(), axis=1)
#         x[0][0] += list(L[:, 0] - logsumexp(L[:, 1:], axis=1))
#         x[1][0] += list(logsumexp(model.L, b=model.is_chosen, axis=1) - logsumexp(model.L, b=1 - model.is_chosen, axis=1))
#         L = -np.sort(-model.ideal_observer_log_posterior_odds(include_bias=True).to_numpy(), axis=1)
#         x[0][1] += list(L[:, 0] - logsumexp(L[:, 1:], axis=1))
#         L = model.L + np.repeat([0] + list(model.fit().b), model.multiplicity)
#         x[1][1] += list(logsumexp(L, b=model.is_chosen, axis=1) - logsumexp(L, b=1 - model.is_chosen, axis=1))
#         p = data.cross_validate(models.HumanObserver4Param)
#         x[0][2] += list(np.log(p.max(axis=1)) - np.log(1 - p.max(axis=1)))
#         p['choice'] = model.df['choice']
#         x[1][2] += list(p.apply(lambda row: np.log(row[row['choice']]) - np.log(1 - row[row['choice']]), axis=1))
#     xlabels = [
#         [r"log $p$($\bf{X}$|$S$ best) - log $p$($\bf{X}$|$S$ not best)",
#          r"$(b_{S\,best}$+log $p$($\bf{X}$|$S$ best)) - $\sum_{S'\,not\,best} (b_{S'}$ + log $p$($\bf{X}$|$S'$))",
#          r"log $P$($S$ best|$\bf{X}$) - log $P$($S$ not best|$\bf{X}$)"],
#         [r"log $p$($\bf{X}$|$S$ chosen) - log $p$($\bf{X}$|$S$ not chosen)",
#          r"$(b_{S\,chosen}$+log $p$($\bf{X}$|$S$ chosen)) - $\sum_{S'\,not\,chosen} (b_{S'}$ + log $p$($\bf{X}$|$S'$))",
#          r"log $P$(choice=$S$ chosen|$\bf{X}$) - log $P$(choice=$S$ not chosen|$\bf{X}$)"]
#     ]
#     for i in range(2):
#         for j in range(3):
#             _ax = fig.add_subplot(gs[j, i])
#             binned_plot(x[i][j], y, q, _ax)
#             _ax.set_xlabel(xlabels[i][j])
#             _ax.set_ylim(0, 1)
#             _ax.set_yticks([0, 1])
#             _ax.set_yticklabels(['Low', 'High'])
#             _ax.set_ylabel('Avg. reported\nconfidence', labelpad=-16)
...
def model_against_chance(ax: plt.Axes):
    from pub.eLife.plot_fig4 import Models, L
...
if __name__ == '__main__':
    ...
    import pub.eLife.matplotlib_config
